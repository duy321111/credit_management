...
def test_add_admin():
    ...
    print("SUCCESS add_admin, gas used:", receipt.gasUsed)
...
def test_create_loan():
    ...
    receipt = send_tx(
        user_account,
        contract.functions.create_loan("Nguyen Van A", id_hash, 1000000, 5, 6)
    )
...
def test_pay_loan():
    ...
    principal = 1_000_000
    interest = (1_000_000 * 5 * 6) // 100  # = 300,000
    total = principal + interest

    receipt = send_tx(
        user_account,
        contract.functions.pay_loan(1),
        value=total
    )
...
def test_get_history():
    ...
    for loan in loans:
        print(f"LoanID={loan[0]}, Status={loan[7]}, Amount={loan[4]}")
